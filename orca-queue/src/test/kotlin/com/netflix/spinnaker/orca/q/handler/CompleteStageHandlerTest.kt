...
import com.netflix.spinnaker.orca.DefaultTaskImplementationResolver
      ...
      given("some synthetic stages were skipped") {
        val pipeline = pipeline {
          stage {
            ...
            stageWithSyntheticBefore.buildTasks(this, DefaultTaskImplementationResolver())
          }
        }
      ...
      and("it comes after its parent stage") {
        val pipeline = pipeline {
          stage {
            ...
            stageWithSyntheticAfter.buildTasks(this, DefaultTaskImplementationResolver())
            stageWithSyntheticAfter.buildAfterStages(this)
          }
        }
    ...
    listOf(SUCCEEDED, FAILED_CONTINUE).forEach { status ->
      context("when one branch completes with $status") {
        val pipeline = pipeline {
          stage {
            ...
            stageWithParallelBranches.buildTasks(this, DefaultTaskImplementationResolver())
          }
        }

        val message = pipeline.stageByRef("1<1").let { completedSynthetic ->
          singleTaskStage.buildTasks(completedSynthetic, DefaultTaskImplementationResolver())
      ...
      given("all branches are complete") {
        ...
        pipeline.stages.filter { it.parentStageId != null }.forEach {
          singleTaskStage.buildTasks(it, DefaultTaskImplementationResolver())
